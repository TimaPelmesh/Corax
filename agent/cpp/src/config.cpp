...
AgentConfig load_agent_config() {
  ...
  // Public sidecar configuration. Portable bundles keep the token out of this
  // file; old agent.json files with agent_token remain supported.
  ...
  // New portable flow: a one-time provisioning file is converted to a
  // machine-bound Windows DPAPI credential on first launch.
  auto secure_token = load_or_provision_agent_token(util::exe_dir());
  if (!secure_token.empty()) cfg.agent_token = secure_token;
