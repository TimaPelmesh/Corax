...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/release-2.2.0-0e7c66?style=flat-square" alt="Release 2.2.0" />
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Agents-EXE%20%7C%20ZIP-111827?style=flat-square" alt="Agents" />
</p>
...
|------|------------------|
| **Сбор** | Агенты Win (**EXE C++** или ZIP / PowerShell) отдают железо, ПО, периферию по LAN |
...
**Автор:** Иванов Тимур · **Лицензия:** [GNU GPL v3](LICENSE) · © 2026 · [CONTRIBUTING](CONTRIBUTING.md) · [CHANGELOG](CHANGELOG.md) · [RELEASE NOTES 2.2.0](RELEASE_NOTES_v2.2.0.md)
...
| Веб | свой пароль, LDAP, Bitrix, HTTPS, **сборка агентов (EXE / ZIP)**, бэкап |
| `.env` + restart | `CORAX_ADVERTISE_HOST`, `CORS_ORIGINS` (LAN; при HTTPS — и `https://…`) |
...
# Если включён HTTPS (Local CA / enterprise), добавьте те же origin со схемой https:
# CORS_ORIGINS=…,https://192.168.x.x:3000,https://localhost:3000
```

```bash
npm run docker:restart
# Фаервол: sudo ufw allow 3000/tcp
# Агенты: Настройки → Сборка агента → EXE (рекомендуется) или ZIP
```

### HTTPS (тот же порт `:3000`)

Один порт = одна схема. Режимы в **Настройки → HTTPS** (`/settings/https`):

| Режим | URL | Доверие |
|-------|-----|--------|
| HTTP (LAN) | `http://IP:3000` | не нужно |
| HTTPS + CORAX Local CA | `https://IP:3000` | установить `ca.crt` на ПК (`scripts/install-corax-ca.bat /machine` или GPO) |
| HTTPS + корпоративный CA | `https://IP:3000` | корень AD уже на машинах; leaf+key импорт в UI |

После смены режима: **`npm run docker:restart`**, затем **пересоберите агентов** (схема `http`/`https` берётся из TLS-статуса).  
Health внутри Docker после TLS: compose пробует `https` (`curl -k`) и `http` на `/api/v1/health/ready`.  
Подробности: [deploy/DOCKER.md — HTTPS](deploy/DOCKER.md#https).

#### Важно: куки после смены HTTP ↔ HTTPS

При **HTTPS** браузер ставит куки `access_token` и `csrf_token` с флагом **`Secure`**. Если потом переключили панель обратно на **HTTP** (или открыли тот же хост по `http://`, а куки остались от `https://`), браузер **отклоняет** новые куки без `Secure`: в консоли будет что-то вроде «кука … была отклонена, так как кука secure уже существует». Вход «не работает», хотя логин/пароль верные — это не поломка сервера, а конфликт кук в браузере.

Что сделать:

1. Открывать панель **той же схемой**, что слушает сервер (после Local CA — только `https://…:3000`).
2. Либо **очистить куки** для этого сайта (Chrome: значок замка / «Файлы cookie и данные сайтов» → удалить для `IP:3000` или localhost) → жёсткое обновление → войти снова.
3. Предупреждение браузера «незащищённая http:// страница с полем пароля» на чистом LAN-HTTP — ожидаемо; для прод в LAN лучше Local CA / HTTPS.
...
- Сборка агентов: **EXE (C++, рекомендуется)** и ZIP (PowerShell) из веб-панели — URL сервера и токен прописываются при сборке; схема `http`/`https` — по режиму TLS.
...
|-----------|------------|
| **EXE C++** (`agent/cpp`) | Рекомендуемый агент Win10/11 — сборка из панели |
| PowerShell v3 (`agent/v3/win10`) | Альтернатива → ZIP из панели |
...
Локальный **`npm start`** (без Docker): UI `:3000`, API `:3001`.
...
Для **Docker / production** health только на **:3000**:
...
Или одной командой (то же самое: `ENVIRONMENT=production`, `PORT=3000`):
...
- Health: `http://<IP-сервера>:3000/api/v1/health/ready`

Рекомендации:

- Сильные `SECRET_KEY`, `AGENT_TOKEN_PEPPER`, пароль bootstrap.
- `CORS_ORIGINS` с реальным URL панели (`http://…` и при TLS — `https://…`).
- HTTPS: встроенный (Настройки → HTTPS: HTTP / Local CA / корпоративный CA) или внешний Caddy/Nginx + `X-Forwarded-Proto: https`. Один порт = одна схема; после смены режима — restart и **пересборка агентов**.
- Регулярный бэкап: `pg_dump -Fc -U inventory inventory > backup.dump` (в Docker — sidecar `db-backup`).
...
#### LibreOffice (экспорт карт)

Нужен только если на bare-metal экспортируете карты через soffice. **Импорт Visio в CORAX отключён.** В Docker-образе Cairo уже есть; LibreOffice не обязателен.

Windows — [LibreOffice](https://www.libreoffice.org/), в `.env`:
...
- `SOFFICE_PATH` — путь к LibreOffice `soffice` (опционально, экспорт карт; импорт Visio отключён).
...
- `/knowledge-base/warehouse` — склад.
- `/users` — пользователи.
- `/settings/ldap` — LDAP.
- `/settings/bitrix24` — Bitrix24.
- `/settings/agent-tokens` — токены агентов.
- `/settings/agent-bundle` — сборка агентов (EXE C++ / ZIP).
- `/settings/https` — режимы HTTP / Local CA / корпоративный CA.
...
Перед выпуском в production (канон — **Docker**, см. [deploy/DOCKER.md](deploy/DOCKER.md)):

- [ ] `npm run docker:up` (или bare-metal: `ENVIRONMENT=production`, `PORT=3000`)
- [ ] Уникальные `SECRET_KEY` (≥32), `AGENT_TOKEN_PEPPER`, `BOOTSTRAP_ADMIN_PASSWORD` (≥12) — не дефолты
- [ ] `backend/.env` не в Git; пароль admin сменён после первого входа
- [ ] `CORAX_ADVERTISE_HOST` = LAN-IP ВМ; `CORS_ORIGINS` включает `http://<LAN-IP>:3000` (и `https://…` при TLS)
- [ ] Firewall: только **`:3000`** (+ SSH); Postgres не в LAN (`127.0.0.1:5433`)
- [ ] Health: `curl -fsS http://127.0.0.1:3000/api/v1/health/ready` (при HTTPS: `curl -fsSk https://127.0.0.1:3000/api/v1/health/ready`)
- [ ] HTTPS (опционально): Настройки → HTTPS → Local CA / enterprise → `docker:restart` → CA на ПК → **пересборка EXE/ZIP**
- [ ] Агенты: сборка с LAN-IP (не `127.0.0.1` / не `172.x`); токены через UI
- [ ] Бэкап: volume `corax_backups` / UI restore проверен
- [ ] Smoke: вход, дашборд, отчёт агента, заявка, карта (если используете)
...
LibreOffice (опционально, экспорт карт; импорт Visio отключён):
...
7. Проверить `GET /api/v1/health/ready` на **:3000**, вход, дашборд
...
Healthcheck (**Docker / production** — порт **3000**):
...
Локальный `npm start` (dev): health API на `http://127.0.0.1:3001/api/v1/health`.
## Лицензия
