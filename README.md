...
12. [Linux: установка с нуля, systemd, cron](#linux-установка-с-нуля-systemd-cron)
13. [Эксплуатация и бэкапы](#эксплуатация)


## Docker (рекомендуемый способ запуска)

Подробно: **[deploy/DOCKER.md](deploy/DOCKER.md)** — обоснование стека, volumes, HTTPS, restore, логи.
...
Одна команда после clone (создаст `backend/.env` с сильными секретами, если файла нет):

```bash
npm run docker:up       # init .env + db + app + backup
...
Логин/пароль bootstrap печатаются при **первом** `docker:up` и сохраняются в `backend/.docker-credentials` (не в Git). После входа смените пароль в панели.

| Где | Что настраивать |
|-----|-----------------|
| Авто в `.env` | `SECRET_KEY`, pepper, пароль Postgres, bootstrap-пароль |
| Веб после входа | Свой пароль, LDAP, Bitrix24, HTTPS, токены/сборка агентов, бэкап БД |
| Правка `.env` + `docker:restart` | CORS / LAN IP, `CORAX_ADVERTISE_HOST`, порты, LM Studio |

Откройте `http://127.0.0.1:3000/`. Агентам: `http://<LAN-IP>:3000`.  
Логи контейнера: `npm run docker:logs`. Файлы внутри volume: `/data/logs/corax.jsonl`.
