...
10. [Эксплуатация и бэкапы](#эксплуатация)
...
- Пользователи и роли: локальные учётные записи, LDAP-аутентификация/синхронизация, роли `observer`, `editor`, `admin`.
...
| Active Directory / LDAP | Вход и синхронизация пользователей |
...
Файл `/etc/systemd/system/corax.service`:

```ini
[Unit]
Description=CORAX API
...
sudo systemctl enable corax
sudo systemctl start corax
```
...
- fallback-аутентификация: если локальный пароль не подошёл, backend пробует LDAP.
...
- `backend/.env.example`;
- `run.py`, `start_all.py`, `start_all.bat`, `dev.bat`, `scripts/ensure_postgres.py`, `scripts/migrate_sqlite_to_postgres.py`;
