...
function displayTitle(row: NetworkPrinter) {
  ...
  if (name) return name
  return row.snmp_model?.trim() || row.ip_address || 'Printer'
}

function displaySnmpHint(row: NetworkPrinter) {
...
function PrinterNameCell({
  ...
  const hint = displaySnmpHint(row)

  return (
    <div className="group/name min-w-0" onClick={(e) => editing && e.stopPropagation()}>
      <div className="flex min-w-0 items-center gap-1.5">
        <div className="min-w-0 flex-1">
          {editing ? (
            <input
              className="app-input !min-h-[2rem] !py-1 text-sm font-medium"
              value={draft}
              autoFocus
              disabled={saving}
            ...
            <button
              type="button"
              className="w-full whitespace-normal break-words text-left font-medium leading-snug text-[var(--color-fg)] hover:text-blue-700"
              title={title}
              onClick={(e) => {
                e.stopPropagation()
                onOpen()
          ...
          >
            {editing ? (
              <>
                <button
                  type="button"
                  className="app-btn app-btn-primary !min-h-0 !px-2 !py-1 text-[11px] leading-none"
                ...
                <button
                  type="button"
                  className="app-btn app-btn-secondary !min-h-0 !px-2 !py-1 text-[11px] leading-none"
                  disabled={saving}
                  onClick={onCancel}
                >
                  {t('common.cancel')}
              ...
              <button
                type="button"
                className="rounded-md p-1 text-[var(--color-fg-subtle)] opacity-70 transition hover:bg-blue-50 hover:text-blue-700 hover:opacity-100 focus-visible:opacity-100 group-hover/name:opacity-100"
      ...
      {hint ? (
        <div className="mt-0.5 whitespace-normal break-words text-xs leading-snug text-[var(--color-fg-muted)]" title={hint}>
          ({hint})
        </div>
      ) : null}
...
export function PrintersPage() {
  ...
  const startRename = (row: NetworkPrinter) => {
    ...
    setEditingNameDraft(row.name?.trim() || displayTitle(row))
  }
  ...
  const saveRename = async () => {
    if (!canEdit || editingNameId == null) return
    const nm = editingNameDraft.trim()
    if (!nm) {
    ...
    try {
      const updated = await api.patchPrinter(editingNameId, { name: nm })
      setRows((prev) => prev.map((r) => (r.id === updated.id ? updated : r)))
      if (detailPrinter?.id === updated.id) setDetailPrinter(updated)
      setEditingNameId(null)
      setEditingNameDraft('')
    ...
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-4">
      <header className="flex flex-wrap items-center justify-end gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <div
            className="inline-flex max-w-[min(100%,18rem)] items-center gap-1.5 rounded-lg bg-[var(--color-bg-muted)] px-3 py-1.5 text-[11px] font-medium text-[var(--color-fg-muted)]"
            ...
            {colsOpen ? (
              <div className="absolute right-0 z-40 mt-1.5 w-[min(18rem,calc(100vw-2rem))] rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-2 shadow-xl">
                ...
                <input
                  ...
                  className="mb-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-1.5 text-xs"
                  autoFocus
                />
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        <span className="rounded-lg bg-emerald-500/10 px-3 py-1.5 text-emerald-800 dark:text-emerald-200">
          {t('printers.available')}: <strong>{stats.online}</strong>
        </span>
        <span className="rounded-lg bg-[var(--color-bg-muted)] px-3 py-1.5">
          {t('printers.snmpOk')}: <strong>{stats.snmpOk}</strong>
        </span>
        <span
        ...
        >
          {t('printers.lowToner')}: <strong>{stats.lowToner}</strong>
        </span>
      </div>
                ...
                filteredRows.map((r) => {
                  ...
                  const pollBadge =
                    r.poll_status === 'online'
                      ? { text: t('printers.statusOnline'), cls: 'bg-[var(--color-surface-muted)] text-[var(--color-fg)] ring-[var(--color-border)]' }
                      : r.poll_status === 'offline'
                        ? { text: t('printers.statusOffline'), cls: 'bg-[var(--color-surface-muted)] text-amber-800 ring-[var(--color-border)] dark:text-amber-200' }
                        : { text: t('printers.statusUnknown'), cls: 'bg-[var(--color-surface-muted)] text-[var(--color-fg-muted)] ring-[var(--color-border)]' }
                  return (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      {visibleCols.model ? (
                        <td
                          className="app-table-sticky-col min-w-[16rem] max-w-[22rem] px-4 py-3 align-top"
                          onClick={(e) => {
                            if (editingNameId === r.id) e.stopPropagation()
                          }}
                        >
                          <PrinterNameCell
                            row={r}
                            canEdit={canEdit}
                            editing={editingNameId === r.id}
                            draft={editingNameDraft}
                            saving={nameSaveBusy}
                            onStartEdit={() => startRename(r)}
                            onDraftChange={setEditingNameDraft}
                            onSave={() => void saveRename()}
                            onCancel={cancelRename}
                            onOpen={() => setDetailPrinter(r)}
                          />
                        </td>
                      ) : null}
                      {visibleCols.ip ? (
