...
function TreeNodeRow({
  ...
  return (
    <div className="relative">
      {depth > 0 ? (
        <>
          <span
            ...
            style={{ left: `${indentPx - 11}px`, height: isLast ? '1.1rem' : '100%' }}
            aria-hidden
          />
          <span
            className="pointer-events-none absolute top-[0.95rem] h-px w-2.5 bg-[var(--color-border)]"
      ...
      <div
        className="group/row relative mb-0.5 flex items-center gap-1.5 rounded-md px-1.5 py-1 transition hover:bg-[var(--color-surface-muted)]"
      ...
      >
        ...
        >
          {hasChildren ? (
            <IconChevron open={expanded} className="h-3.5 w-3.5" />
          ) : (
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <div className="min-w-0 flex-1">
            {canManage ? (
              <input
                ...
                disabled={saving}
                className="w-full min-w-0 border-0 bg-transparent p-0 text-sm font-medium text-[var(--color-fg)] outline-none ring-0 placeholder:text-[var(--color-fg-subtle)] focus:ring-0 disabled:opacity-60"
                spellCheck={false}
                aria-label={t('settingsCategories.nameAria', { path: node.path })}
              />
            ) : (
              <div className="text-sm font-medium text-[var(--color-fg)]">{node.name}</div>
            )}
            <div className="mt-0.5 truncate text-[10px] text-[var(--color-fg-subtle)]" title={node.path}>
              {node.path}
              {hasChildren ? (
                <span className="ml-1.5 text-[var(--color-fg-muted)]">
          ...
          {canManage ? (
            <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition group-hover/row:opacity-100 focus-within:opacity-100">
              <button
                type="button"
                className="rounded px-1.5 py-0.5 text-[11px] font-medium text-[var(--color-fg-subtle)] hover:bg-[var(--color-surface)] hover:text-[var(--color-fg)]"
              ...
              <button
                type="button"
                className="rounded px-1.5 py-0.5 text-[11px] font-medium text-[var(--color-fg-muted)] hover:bg-[var(--color-surface)] hover:text-[var(--color-fg)]"
              ...
              <button
                type="button"
                className="inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--color-fg-subtle)] hover:bg-[var(--color-surface)] hover:text-[var(--color-fg)]"
                aria-label={t('settingsCategories.deleteAction')}
      ...
      {rowErr ? (
        <div className="mb-1 text-xs text-[var(--color-error-fg)]" style={{ marginLeft: `${indentPx + 2.25}rem` }}>
          {rowErr}
        </div>
      ) : null}
      ...
      {isAddingHere && canManage ? (
        <form
          className="mb-1.5 flex flex-wrap items-center gap-2 px-1.5 py-1.5"
          style={{ marginLeft: `${indentPx + 1.75}rem` }}
          onSubmit={(e) => {
            e.preventDefault()
        ...
        >
          <input
            ...
            className="app-input !min-h-8 min-w-[10rem] flex-1 py-1 text-sm"
            disabled={addChildBusy}
            onKeyDown={(e) => {
              if (e.key === 'Escape') onCancelAddChild()
            }}
          />
          <button type="submit" className="app-btn app-btn-primary !min-h-8 px-2.5 py-1 text-xs" disabled={addChildBusy || !addChildDraft.trim()}>
            {addChildBusy ? t('settingsCategories.addChildBusy') : t('settingsCategories.addChildButton')}
          </button>
          <button type="button" className="rounded-md px-2 py-1 text-xs text-[var(--color-fg-muted)] hover:text-[var(--color-fg)]" onClick={onCancelAddChild} disabled={addChildBusy}>
            {t('common.cancel')}
          </button>
        </form>
      ) : null}
...
export function SettingsCategoriesPage() {
  ...
  return (
    <div>
      {canManage ? (
        <div className="mb-4 flex max-w-xl flex-wrap items-center gap-2">
          <input
            ...
            disabled={rootBusy}
            aria-label={t('settingsCategories.newRootLabel')}
          />
          <button
            type="button"
            onClick={() => void addRoot()}
            className="app-btn app-btn-primary !min-h-9 px-3 text-sm"
      ...
      <div className="max-w-3xl overflow-hidden rounded-xl border border-[var(--color-border)]">
        <div className="flex flex-wrap items-center gap-2 border-b border-[var(--color-border)] px-3 py-2">
          <div className="min-w-0 flex-1 text-xs text-[var(--color-fg-subtle)]">
            {loading ? (
              t('common.loading')
            ) : search.trim() ? (
          ...
          <input
            ...
            className="app-input !min-h-8 max-w-[12rem] flex-1 py-1 text-xs sm:max-w-xs"
            aria-label={t('settingsCategories.searchAria')}
          />
          <button
            type="button"
            className="rounded-md px-2 py-1 text-xs font-medium text-[var(--color-fg-muted)] transition hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-fg)]"
            onClick={expandAll}
        ...
        {loading ? (
          ...
          <div className="bg-[var(--color-surface)] px-1.5 py-2 sm:px-2">
