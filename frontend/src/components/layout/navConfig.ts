...
export function buildNavSections(user: { is_superuser?: boolean; role?: string } | null): NavSectionDef[] {
  const sections: NavSectionDef[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { to: '/requests/templates', end: true, icon: IconFolder, labelKey: 'nav.requestTemplates' },
        { to: '/requests/stats', end: true, icon: IconChart, labelKey: 'nav.requestStats' },
      ],
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: IconInfo,
          labelKey: 'nav.guide',
          keywords: ['руководство', 'документация', 'справка', 'инструкция', 'guide', 'docs', 'help'],
        },
        {
  ...
  if (user?.is_superuser || user?.role === 'editor') {
    settingsItems.push(
      ...
      { to: '/settings/categories', icon: IconTree, labelKey: 'nav.categories', keywords: ['заявки', 'tickets'] },
    )
    if (user?.is_superuser) {
      settingsItems.push(
        { to: '/users', icon: IconUsers, labelKey: 'nav.users' },
        { to: '/settings/ldap', icon: IconUser, labelKey: 'nav.ldap' },
        { to: '/settings/bitrix24', icon: IconSend, labelKey: 'nav.bitrix24' },
        { to: '/settings/zabbix', icon: IconSignal, labelKey: 'nav.zabbix' },
        {
        ...
        { to: '/settings/agent-tokens', icon: IconKey, labelKey: 'nav.agentTokens' },
        {
          ...
          keywords: ['zip', 'exe', 'deploy', 'пакет', 'агент', 'agent', 'tray'],
        },
        {
          to: '/settings/wol',
          icon: IconPower,
