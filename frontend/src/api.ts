...
/** WikiRAG: бэкенд ждёт LM Studio / Ollama до lm_studio_timeout_seconds (по умолчанию 300 с). */
...
export type FloorLayoutSettings = {
  pc_hover_card?: {
...
export type CatalogFilterItem = { kind: CatalogKind; name: string }

export type CatalogFilterHosts = {
...
export type DashboardSummary = {
  ...
  /** Auto-bucketed closed tickets over time (day / week / month). */
  service_requests_closed_granularity?: 'day' | 'week' | 'month' | string
  service_requests_closed_series?: DashboardTimePoint[]
  ...
  upcoming_notes?: DashboardUpcomingNote[]
  /** Notes the current user can access (own + shared; all for superuser). */
  notes_total?: number
}
...
export type DashboardUpcomingNote = {
  id: number
  title: string
  plan_start?: string | null
  plan_end?: string | null
...
export type RiskComputer = {
  ...
  antivirus_status: 'protected' | 'attention' | 'unknown'
...
export type RiskOverview = {
  ...
  computers_total: number
  computers_critical: number
  computers_high: number
  computers_medium: number
  computers_healthy: number
  antivirus_protected: number
  antivirus_attention: number
...
export type TicketHandlerConfig = {
  ...
  llm_provider: 'ollama' | 'lm_studio' | string
  llm_base_url: string
  llm_model: string
  include_corax_knowledge: boolean
  include_wiki_docs: boolean
...
export type TicketHandlerStatsPoint = {
  date: string
  total: number
  created_ticket: number
  skipped_ticket: number
  error: number
  ok: number
}

export type TicketHandlerStats = {
  total: number
  created_ticket: number
  skipped_ticket: number
  error: number
  ...
  series: TicketHandlerStatsPoint[]
}

export type TicketHandlerPublicContext = {
  enabled: boolean
  hostname: string
  computer_id: number | null
  location: string | null
...
export function normalizeDashboardSummary(raw: DashboardSummary): DashboardSummary {
  ...
  return {
    ...
    service_requests_closed_granularity: raw.service_requests_closed_granularity || 'day',
    service_requests_closed_series: Array.isArray(raw.service_requests_closed_series)
      ? raw.service_requests_closed_series.map((p) => ({
          date: String(p.date),
    ...
    upcoming_notes: Array.isArray(raw.upcoming_notes) ? raw.upcoming_notes : [],
    notes_total: raw.notes_total ?? 0,
  }
}

export type SoftwareInstallHosts = {
  name: string
  hostnames: string[]
  total?: number
  items?: DashboardSegmentComputer[]
...
export type SearchResult = {
  ...
  snippet: string | null
  score: number
  match_kind: 'exact' | 'prefix' | 'partial' | 'full_text' | 'fuzzy'
...
export const api = {
  ...
  searchCatalog: (q: string, opts?: { types?: string[]; limit?: number }) => {
    ...
    for (const type of opts?.types ?? []) p.append('types', type)
    if (opts?.limit != null) p.set('limit', String(opts.limit))
    return request<SearchResponse>(`${API_PREFIX}/search?${p}`)
  },

  reindexCatalog: () =>
    request<{ ok: boolean; indexed: Record<string, number> }>(`${API_PREFIX}/search/reindex`, {
      method: 'POST',
    }),
  ...
  dashboardNavBadges: () => request<DashboardNavBadges>(`${API_PREFIX}/dashboard/nav-badges`),

  dashboardCalendar: (month: string) =>
    request<DashboardCalendarItem[]>(`${API_PREFIX}/dashboard/calendar?month=${encodeURIComponent(month)}`),

  riskOverview: () => request<RiskOverview>(`${API_PREFIX}/risks/overview`),

  riskHistory: (limit?: number) => {
    const p = new URLSearchParams()
    if (limit != null) p.set('limit', String(limit))
    const qs = p.toString()
    return request<RiskHistory>(`${API_PREFIX}/risks/history${qs ? `?${qs}` : ''}`)
  },

  riskFindingAction: (body: {
    finding_id: string
    status: 'open' | 'acknowledged' | 'ignored'
    note?: string | null
  }) =>
    request<{ ok: boolean; finding_id: string; status: string }>(`${API_PREFIX}/risks/findings/actions`, {
      method: 'POST',
      json: body,
    }),
  ...
  catalogFilterHosts: (filters: CatalogFilterItem[], limit?: number) =>
    request<CatalogFilterHosts>(`${API_PREFIX}/dashboard/catalog-filter-hosts`, {
      method: 'POST',
      json: { filters, limit: limit ?? 500 },
    }),
  ...
  notes: () => request<NoteListItem[]>(`${API_PREFIX}/notes`),

  note: (id: number) => request<NoteRow>(`${API_PREFIX}/notes/${id}`),
  ...
  replaceNoteShares: (id: number, shares: { user_id: number; can_edit: boolean }[]) =>
    request<NoteRow>(`${API_PREFIX}/notes/${id}/shares`, { method: 'PUT', json: { shares } }),
  ...
  updateTicketHandlerConfig: (body: Partial<TicketHandlerConfig>) =>
    request<TicketHandlerConfig>(`${API_PREFIX}/ticket-handler/config`, { method: 'PUT', json: body }),

  regenerateTicketHandlerSecret: () =>
    request<TicketHandlerConfig>(`${API_PREFIX}/ticket-handler/config/regenerate-secret`, {
      method: 'POST',
    }),

  ticketHandlerStats: (params?: { from?: string; to?: string }) => {
    const q = new URLSearchParams()
    if (params?.from) q.set('from', params.from)
    if (params?.to) q.set('to', params.to)
    const qs = q.toString()
    return request<TicketHandlerStats>(`${API_PREFIX}/ticket-handler/stats${qs ? `?${qs}` : ''}`)
  },
  ...
  ticketHandlerIntake: (body: {
    ...
    request<TicketHandlerIntakeResult>(`${API_PREFIX}/ticket-handler/intake`, {
      method: 'POST',
      json: body,
    }),

  ticketHandlerSandbox: (body: {
    hostname: string
    title: string
    description?: string
    dry_run?: boolean
  }) =>
    request<TicketHandlerIntakeResult>(`${API_PREFIX}/ticket-handler/sandbox`, {
      method: 'POST',
      json: body,
    }),
  ...
  serviceRequests: (opts?: { status?: string; limit?: number; skip?: number }) => {
    ...
    if (opts?.limit != null) p.set('limit', String(opts.limit))
    if (opts?.skip != null && opts.skip > 0) p.set('skip', String(opts.skip))
    const qs = p.toString()
  ...
  importServiceRequestsGlpiCsv: async (file: File) => {
    ...
    return (await res.json()) as ServiceRequestImportStatus
  },

  importServiceRequestsJson: async (file: File) => {
    const fd = new FormData()
    fd.set('file', file)
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    if (csrf) headers.set('X-CSRF-Token', csrf)
    const res = await fetch(apiUrl(`${API_PREFIX}/service-requests/import-json`), {
      method: 'POST',
      credentials: 'include',
      headers,
      body: fd,
  ...
  serviceRequestImportStatus: () =>
    request<ServiceRequestImportStatus>(`${API_PREFIX}/service-requests/import-status`),
  ...
  wikiRagFolders: () => request<{ folders: string[] }>(`${API_PREFIX}/wiki-rag/folders`),

  createWikiRagFolder: (path: string) =>
    request<{ path: string; ok: boolean; deleted_documents?: number }>(`${API_PREFIX}/wiki-rag/folders`, {
      method: 'POST',
      json: { path },
    }),

  renameWikiRagFolder: (fromPath: string, toPath: string) =>
    request<{ path: string; ok: boolean }>(`${API_PREFIX}/wiki-rag/folders`, {
      method: 'PATCH',
  ...
  deleteWikiRagFolder: (path: string, opts?: { recursive?: boolean }) =>
    request<{ path: string; ok: boolean; deleted_documents?: number }>(
      `${API_PREFIX}/wiki-rag/folders/delete`,
      {
        method: 'POST',
        json: { path, recursive: opts?.recursive !== false },
        // Large folder trees (hundreds of files) need more than the default 25s.
        timeout_ms: 180_000,
      },
    ),

  exportWikiRagIndex: async (): Promise<void> => {
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    if (csrf) headers.set('X-CSRF-Token', csrf)
    const res = await fetch(apiUrl(`${API_PREFIX}/wiki-rag/export-index`), {
      credentials: 'include',
      headers,
    })
    if (!res.ok) {
    ...
    const cd = res.headers.get('Content-Disposition') || ''
    const m = /filename="?([^";]+)"?/i.exec(cd)
    const filename = m?.[1] || `corax-wikirag-index_${new Date().toISOString().slice(0, 10)}.zip`
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = filename
  ...
  importWikiRagIndex: async (file: File) => {
    const fd = new FormData()
    fd.set('file', file)
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    if (csrf) headers.set('X-CSRF-Token', csrf)
    const res = await fetch(apiUrl(`${API_PREFIX}/wiki-rag/import-index`), {
      method: 'POST',
      credentials: 'include',
      headers,
      body: fd,
  ...
  reindexWikiRagDocument: (id: number) =>
    request<{ ok: boolean; document_id?: number; detail?: string | null }>(
      `${API_PREFIX}/wiki-rag/${id}/reindex`,
      { method: 'POST' },
    ),

  reindexAllWikiRagDocuments: () =>
    request<{ ok: boolean; total?: number; detail?: string | null }>(`${API_PREFIX}/wiki-rag/reindex-all`, {
      method: 'POST',
    }),

  reindexPendingWikiRagDocuments: () =>
    request<{ ok: boolean; total?: number; detail?: string | null }>(`${API_PREFIX}/wiki-rag/reindex-pending`, {
      method: 'POST',
    }),

  uploadWikiRagDocument: async (file: File, comment?: string | null, relativePath?: string | null) => {
    const fd = new FormData()
    fd.set('file', file)
    ...
    if (rel) fd.set('relative_path', rel.replace(/\\/g, '/'))
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    if (csrf) headers.set('X-CSRF-Token', csrf)
  ...
  updateWikiRagDocument: (id: number, body: { comment?: string | null; original_filename?: string | null }) =>
  ...
  deleteWarehouseRoom: (id: number, opts?: { purge?: boolean }) => {
    const qs = opts?.purge ? '?purge=true' : ''
    return request<void>(`${API_PREFIX}/warehouse/rooms/${id}${qs}`, { method: 'DELETE' })
  },
  ...
  writeOffWarehouseItem: (id: number, opts?: { comment?: string | null; quantity?: number | null }) => {
    const sp = new URLSearchParams()
    if (opts?.comment) sp.set('comment', opts.comment)
    if (opts?.quantity != null) sp.set('quantity', String(opts.quantity))
    const qs = sp.toString()
    return request<WarehouseStockItem>(
      `${API_PREFIX}/warehouse/items/${id}/write-off${qs ? `?${qs}` : ''}`,
      { method: 'POST' },
    )
  },
  ...
  deleteWarehouseMovement: (id: number) =>
    request<void>(`${API_PREFIX}/warehouse/movements/${id}`, { method: 'DELETE' }),

  clearWarehouseMovements: (roomId?: number | null) => {
    const sp = new URLSearchParams()
    if (roomId != null) sp.set('room_id', String(roomId))
    const qs = sp.toString()
    return request<{ ok: boolean; deleted: number }>(
      `${API_PREFIX}/warehouse/movements${qs ? `?${qs}` : ''}`,
      { method: 'DELETE' },
    )
  },
  ...
  exportWarehouseCsv: async (roomId?: number | null) => {
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    if (csrf) headers.set('X-CSRF-Token', csrf)
    const sp = new URLSearchParams()
    if (roomId != null) sp.set('room_id', String(roomId))
    const qs = sp.toString()
    const res = await fetch(apiUrl(`${API_PREFIX}/warehouse/export.csv${qs ? `?${qs}` : ''}`), {
      credentials: 'include',
      headers,
    })
    if (!res.ok) {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = 'corax_warehouse.csv'
    a.click()
    URL.revokeObjectURL(url)
  },

  importWarehouseCsv: async (file: File, roomId?: number | null) => {
    const fd = new FormData()
    fd.set('file', file)
    const headers = new Headers()
    const csrf = getCookie('csrf_token')
    ...
    if (roomId != null) sp.set('room_id', String(roomId))
    const qs = sp.toString()
    const res = await fetch(apiUrl(`${API_PREFIX}/warehouse/import.csv${qs ? `?${qs}` : ''}`), {
      method: 'POST',
      credentials: 'include',
      headers,
      body: fd,
  ...
  patchPrinter: (id: number, body: { name?: string | null; location?: string | null; notes?: string | null; ip_address?: string | null }) =>
...
export type WikiRagChatResponse = {
  ...
  meta?: {
    ...
    sources?: WikiRagChatParsed['sources']
    lm_base_url?: string
...
export type ServiceRequestImportStatus = {
  running: boolean
  kind: 'glpi_csv' | 'corax_json' | null
