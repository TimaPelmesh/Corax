...
Полная инструкция также в панели: **База знаний → Документация** (разделы «Агент инвентаризации» и «Linux-агент»).
...
```
corax_send.bat          ← ручной запуск: ждёт конца сбора, показывает status
...
```bat
:: на ПК или с шары — окно держится, пока отчёт не уйдёт
...
Проверка (через ~1 минуту): **Компьютеры** — hostname и «последний отчёт»; ярлык **Оставить заявку** на столе; файл `corax-last-run.txt` со `status: OK`. Если окна сразу не стало и файла нет — отчёт не стартовал.
...
Расписание: `install_schedule.bat` один раз **от администратора**. Он ставит задачу `CORAX-Agent` (SYSTEM, без окна) и **сразу запускает первый отчёт**. Дальше — только по таймеру. Не ставьте в Планировщик голый `corax_send.bat`.
