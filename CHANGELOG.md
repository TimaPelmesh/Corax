...
### Docs and reliability

- README spells out volumes, health checks, and the LAN security contract.
- In-app tab is **Documentation** (was Guide): header, plus Reliability and Security sections.
- GET requests retry on a brief 502/503/504 or network drop. The panel shows a banner when `/api/v1/health` is down.
- Crash screen can retry the UI without a full reload.
- Split the building-map helpers and ticket stats/templates out of the two largest page files.
