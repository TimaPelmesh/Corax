...
# - FastAPI serves frontend/dist. Agents and browsers hit one URL.
...
RUN --mount=type=cache,target=/root/.cache/pip \
    grep -vE '^(pytest|pytest-asyncio)([=<>]|$)' requirements.txt > requirements.prod.txt \
...
RUN chmod +x /entrypoint.sh \
  ...
  && find /app/agent/linux -type f \( -name '*.sh' -o -name 'inventory_agent.sh' -o -name 'corax_send.sh' \) -exec chmod +x {} \; 2>/dev/null || true
