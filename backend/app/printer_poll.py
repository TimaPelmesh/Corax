    ...
    *,
    ...
    if not snap.error:
        ...
        if snap.model:
            ...
            # Имя CORAX (name) пользователь задаёт сам — опрос его не затирает.
            # Только первичное заполнение, если имя пустое/похоже на IP.
            current = (row.name or "").strip()
            if not current or current == (row.ip_address or "").strip():
        ...
        if getattr(snap, "serial_number", None):
            row.serial_number = snap.serial_number
        if getattr(snap, "sys_name", None):
