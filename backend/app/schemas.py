...
class ComputerMapItem(BaseModel):
    """Lean row for building map / bind pickers (no raw payload or counts)."""
...
class DashboardTimePoint(BaseModel):
    """Bucket on the closed-tickets timeline (ISO date = period start)."""
...
class DashboardNavBadges(BaseModel):
    """Lightweight sidebar counts — not the full dashboard summary."""
...
class DashboardSummary(BaseModel):
    ...
    # Closed tickets over time (auto day/week/month buckets).
    service_requests_closed_granularity: str = "day"
    service_requests_closed_series: list[DashboardTimePoint] = []
    ...
    upcoming_notes: list["DashboardUpcomingNote"] = []
    notes_total: int = 0


class DashboardUpcomingNote(BaseModel):
    id: int
    title: str
...
class CatalogFilterItem(BaseModel):
    kind: str
    name: str = Field(..., min_length=1, max_length=512)


class CatalogFilterHostsRequest(BaseModel):
    filters: list[CatalogFilterItem] = Field(..., min_length=1, max_length=12)
    limit: int = Field(500, ge=1, le=1000)


class CatalogFilterHostsResponse(BaseModel):
    filters: list[CatalogFilterItem]
    total: int
    hostnames: list[str]
    items: list[DashboardSegmentComputer] = []
...
class ServiceRequestOut(BaseModel):
    ...
    category: str | None = None
    location: str | None = None
    ai_title_suggestion: str | None = None
...
class SelfServiceContextOut(BaseModel):
    hostname: str
    computer_id: int
    location: str | None = None
...
class SelfServiceRequestOut(BaseModel):
    ok: bool = True
    request_id: int
    ticket_no: int | None = None


class WikiRagFolderCreate(BaseModel):
    path: str = Field(min_length=1, max_length=400)


class WikiRagFolderRename(BaseModel):
    from_path: str = Field(min_length=1, max_length=400)
    to_path: str = Field(min_length=1, max_length=400)


class WikiRagFolderDelete(BaseModel):
    path: str = Field(min_length=1, max_length=400)
    recursive: bool = True


class WikiRagFolderOut(BaseModel):
    path: str
    ok: bool = True
    deleted_documents: int = 0


class WikiRagFoldersListOut(BaseModel):
...
class WikiRagReindexOut(BaseModel):
    ok: bool = True
...
class WikiRagIndexImportOut(BaseModel):
    ok: bool = True
    documents: int = 0
    chunks: int = 0
    skipped: int = 0
...
class NoteSharesReplace(BaseModel):
    shares: list[NoteSharePut] = Field(default_factory=list)
...
class NoteOut(BaseModel):
    ...
    can_edit: bool = False
    is_owner: bool = False
    shares: list[NoteShareOut] = Field(default_factory=list)
...
class TicketHandlerPipelineStep(BaseModel):
    id: str
    enabled: bool = True
    label: str = ""
...
class TicketHandlerConfigOut(BaseModel):
    ...
    llm_base_url: str
    llm_model: str
    include_corax_knowledge: bool
    include_wiki_docs: bool
...
class TicketHandlerConfigUpdate(BaseModel):
    enabled: bool | None = None
    ...
    include_corax_knowledge: bool | None = None
    include_wiki_docs: bool | None = None
...
class TicketHandlerStatsPoint(BaseModel):
    date: str
    total: int = 0
    created_ticket: int = 0
    skipped_ticket: int = 0
    error: int = 0
    ok: int = 0


class TicketHandlerStatsOut(BaseModel):
    total: int = 0
    created_ticket: int = 0
    skipped_ticket: int = 0
    error: int = 0
    ...
    series: list[TicketHandlerStatsPoint] = Field(default_factory=list)
...
class TicketHandlerIntakeResponse(BaseModel):
    ok: bool
    answer: str
    status: str
...
class ServiceRequestAiSuggestOut(BaseModel):
    ok: bool
    category: str | None = None
