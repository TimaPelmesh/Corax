...
from app.oem_normalize import (
    ...
    system_model_display_label,
)
from app.os_normalize import os_matches_display
from app.physical_disks import (
    disk_rows_from_raw_payload,
    disk_variant_label,
    disk_size_label,
    normalize_media_type,
    physical_disk_matches,
)
...
def physical_disk_variant_matches(raw_payload: str | None, variant_name: str) -> bool:
    return physical_disk_matches(raw_payload, variant_name)
...
def computer_matches_segment(
    ...
    *,
    ...
    if kind == "os":
        return os_matches_display(c.os_name, name)
    ...
    if kind == "motherboard":
        return motherboard_matches_display(c.motherboard_manufacturer, c.motherboard_product, name)
    if kind == "ram":
...
async def fetch_segment_computers(
    ...
    *,
    ...
    if kind == "hostname":
        r = await db.execute(
            select(Computer).options(noload("*")).where(Computer.hostname == name).limit(1)
        )
    ...
    r = await db.execute(select(Computer).options(noload("*")).order_by(Computer.hostname.asc()))
...
def build_segment_computer_row(
    ...
    *,
    ...
    ram_val = c.ram_gb
    try:
        ram_out = float(ram_val) if ram_val is not None else None
    except (TypeError, ValueError):
        ram_out = None
    host = (c.hostname or "").strip() or f"#{c.id}"
    return {
        "id": c.id,
        "hostname": host,
        "os_name": c.os_name,
...
async def fetch_computers_matching_filters(
    db: AsyncSession,
    *,
    filters: list[tuple[str, str]],
    limit: int = 500,
) -> tuple[list[Computer], int]:
    """AND-intersection of catalog segments (e.g. OS + CPU + software)."""
    ...
    for kind, name in cleaned:
        # High internal limit so intersection is not truncated early.
        computers, _total = await fetch_segment_computers(db, kind=kind, name=name, limit=50_000)
        id_sets.append({int(c.id) for c in computers})
    ...
    r = await db.execute(
        select(Computer)
        .options(noload("*"))
        .where(Computer.id.in_(common_ids))
        .order_by(Computer.hostname.asc())
        .limit(limit)
    )
