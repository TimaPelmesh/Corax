...
_WMI_PLACEHOLDER_RE = re.compile(
    ...
    r"not\s+applicable|unknown|undefined|invalid|\bnone\b|null|"
    r"o\.?e\.?m\.?|n/?a|all\s+series|"
...
def motherboard_display_label(manufacturer: str | None, product: str | None) -> str:
    """Подробная подпись мат. платы: «ASUS · PRIME B450-PLUS»."""
    mfr = normalize_manufacturer(manufacturer)
    if mfr is None:
        raw_mfr = (manufacturer or "").strip()
        mfr = raw_mfr if raw_mfr and not is_placeholder_wmi_value(raw_mfr) else None
    prod = normalize_system_model(product)
    if mfr and prod:
        return f"{mfr} · {prod}"
    if prod:
        return prod
...
def motherboard_matches_display(
    manufacturer: str | None,
    product: str | None,
    display_name: str,
) -> bool:
    return motherboard_display_label(manufacturer, product) == display_name


def aggregate_motherboard_counts(
    rows: list[tuple[str | None, str | None, int]],
    *,
    limit: int | None = None,
) -> list[tuple[str, int]]:
    counter: Counter[str] = Counter()
    for mfr, product, count in rows:
        counter[motherboard_display_label(mfr, product)] += int(count)
