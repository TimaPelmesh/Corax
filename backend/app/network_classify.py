...
_VOIP_RE = re.compile(
    r"\b("
    r"yealink|grandstream|polycom|poly\s*vvx|snom|fanvil|gigaset|"
    r"cisco\s*(ip\s*)?phone|sip\s*phone|asterisk|freepbx|elastix|issabel|3cx|"
    r"yeastar|ucm\d{3,}|ip\s*office|callmanager|cucm|"
    r"mitel|avaya|panasonic\s*kx|nec\s*sv|wildix|"
    r"voip|ip\s*pbx|pbx\b|gatekeeper|fxo|fxs|"
    r"analog\s*gateway|voip\s*gateway|spa\d{3,}|"
    r"атс|уатс|мини[\-\s]?атс|телефони"
    r")\b",
    re.I,
)
...
_CONTROLLER_RE = re.compile(
    r"\b("
    r"wifi\s*controller|wireless\s*controller|wlan\s*controller|"
    r"wireless\s*lan\s*controller|\bwlc\b|"
    r"unifi\s*(network\s*)?(controller|application)|omada\s*(software\s*)?controller|"
    r"mobility\s*controller|aruba\s*(mobility\s*)?controller|"
    r"cisco\s*wlc|catalyst\s*9800|aironet\s*controller|"
    r"wireless\s*controller\s*(5508|5520|8540|3504)|"
    r"ruckus\s*(virtual\s*)?smartzone|smartzone|mist\s*(edge|ap)|"
    r"huawei\s*ac\d{4}|airengine\s*ac|"
    r"wifi[\-\s]?контроллер|контроллер\s*wifi|контроллер\s*точки"
    r")\b",
    re.I,
)
...
_SWITCH_RE = re.compile(
    r"\b("
    r"switch|officeconnect|superstack|catalyst|nexus|procurve|"
    r"powerconnect|force10|edgeswitch|unifi\s*switch|usw[- ]|"
    r"dgs-|des-|dgs\d|des\d|tl-sg|tl-sl|gs\d{3,}|sg\d{3,}|"
    r"crs\d|css\d|sw[\-_ ]?\d|comware|vrp|junos|stackable|"
    r"\d{2,3}[gt]?[- ]?(port|poe)|poe\+?\s*switch|"
    r"managed\s*switch|smart\s*switch|gigabit\s*switch|"
    r"eltex|mes[- ]?\d|es[- ]?\d|qtech|snr[- ]?s|"
    r"allied\s*telesis|extreme\s*xos|brocade|icx\d|"
    r"meraki\s*ms|cisco\s*(sg|cbs|cbs\d)|cbs\d{3}|"
    r"huawei\s*s\d{2,4}|s57\d{2}|s53\d{2}|ce\d{4}|"
    r"fs\.com|fs\s*switch|netgear\s*(gs|ms|xs)|"
    r"zyxel\s*(gs|xgs)|dgs-\d{4}|des-\d{4}|"
    r"коммутатор"
    r")\b",
    re.I,
)
_ROUTER_RE = re.compile(
    r"\b("
    r"router|gateway|isr[\d\-]|asr[\d\-]|edge\s*router|edgerouter|"
    r"mikrotik|routeros|ccr\d|rb\d{3,}|hex\b|hap\s*lite|"
    r"vyos|vyatta|opnsense|pfsense|openwrt|dd-?wrt|tomato\s*firmware|"
    r"ios\s*software.*isr|c\d{4}.*software|"
    r"keenetic|zyxel\s*armor|asuswrt|tplink\s*archer|archer\s*[acaxc]|"
    r"udm[- ]|uxg[- ]|dream\s*machine|unifi\s*(security|dream|gateway)|"
    r"meraki\s*mx\d*|vsrx|srx\d|palo\s*alto.*gateway|"
    r"huawei\s*ar\d|ne40|ne8000|bng\b|bras\b|"
    r"dir-\d{3,}|rt-ax|rt-ac|wrt\d|ax\d{2,4}\b|"
    r"wireless\s*router|wifi\s*router|bgp\b|ospf\b|"
    r"маршрутизатор|роутер"
    r")\b",
    re.I,
)
_AP_RE = re.compile(
    r"\b("
    r"access\s*point|wireless\s*ap|\bwap\b|aironet|"
    r"cAP|wAP|uap[- ]|nano\s*(hd|station)|litebeam|powerbeam|wave\s*ap|"
    r"omada\s*(ap|eap)|instant\s*on.*ap|aruba\s*(iap|ap)|eap\d{3,}|cap\s*ac|"
    r"wifi\s*6|wifi6|802\.11[abgnacax]|wlan\s*ap|"
    r"meraki\s*m[rx]\d*|ruckus|zoneflex|r[567]\d{2}[a-z]?|"
    r"engenius|grandstream\s*gwn|gwn\d{3,}|"
    r"air-cap|air-ap|c91[12]\d|"
    r"flexhd|beaconhd|u6[- ]|u7[- ]|ac[- ]?lite|ac[- ]?pro|"
    r"unifi\s*(ap|uap|wifi)|ubiquiti\s*(ap|uap)|"
    r"cambium|epmp|mikrotik\s*(cap|wap|audience)|"
    r"точка\s*доступа|wifi[-_]?ap|точка\s*wifi"
    r")\b",
    re.I,
)
...
_HOST_SWITCH_RE = re.compile(
    r"^(sw|switch|swi|tor|core|access|asw|dsw|acc|dist|agg|leaf|spine|swc)[\d\-_.]",
    re.I,
)
_HOST_ROUTER_RE = re.compile(
    r"^(rtr|router|rt|gw|gateway|edge|br|er|mx|udm|uxg|r\d+)[\d\-_.]",
    re.I,
)
_HOST_AP_RE = re.compile(
    r"^(ap|wap|wifi|wifiap|wlan|uap|vap|точки?)[\d\-_.]",
    re.I,
)
...
_HOST_CTRL_RE = re.compile(
    r"^(wlc|controller|ctrl|unifi[\-_]?ctrl|omada[\-_]?ctrl|wifi[\-_]?ctrl)[\d\-_.]",
    re.I,
)
...
_VENDOR_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bgrandstream\b", re.I), "Grandstream"),
    (re.compile(r"\bvmware\b|esxi", re.I), "VMware"),
    (re.compile(r"keenetic", re.I), "Keenetic"),
    (re.compile(r"\bmeraki\b", re.I), "Cisco Meraki"),
...
def classify_device(
    ...
    *,
    ...
    if vendor in _INFRA_VENDORS:
        ...
        if vendor == "MikroTik":
            scores["router"] += 0.8
        if vendor in {"Ubiquiti", "Ruckus", "EnGenius"}:
            scores["ap"] += 0.6
        ...
        if vendor in {"Yealink", "Grandstream"}:
            scores["voip"] += 1.6
            scores["ap"] += 0.2  # GWN APs also exist
        if vendor == "VMware":
            scores["server"] += 1.5
        if vendor == "TP-Link":
            scores["ap"] += 0.35
    ...
    if hints.wifi_ports >= 1:
        scores["ap"] += 2.2 + min(hints.wifi_ports, 4) * 0.35
        signals.append(f"wifi_ifaces:{hints.wifi_ports}")
        # Few ethernet + wifi → almost certainly AP, not controller
        if hints.ethernet_ports <= 2 and hints.wifi_ports >= 1:
            scores["ap"] += 1.2
            scores["controller"] -= 0.5
            signals.append("ap_form_factor")
        # Many ethernet + wifi naming → controller / L3 gateway
        if hints.ethernet_ports >= 4 and hints.wifi_ports >= 1:
            scores["controller"] += 0.8
            signals.append("ctrl_candidate")
