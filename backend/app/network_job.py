...
class NetworkJobRunner:
    ...
    async def _run_discover(self, db) -> None:
        ...
        if trace.routes or trace.gateways_created:
            msg += f" Трассы: {trace.routes}, шлюзы: {trace.gateways_created}."

        self._set(phase="links", progress=92, message="Построение топологии…")
        from app.network_link_builder import rebuild_all_links

        links = await rebuild_all_links(db)
        msg += f" {links.message}" if links.message else ""
        ...
        self.state.last_result = {
            ...
            "trace_routes": trace.routes,
            "trace_gateways": trace.gateways_created,
            "links_devices": links.device_links,
            "links_computers": links.computer_links,
    ...
    async def _run_poll(self, db) -> None:
        ...
        self.state.last_result = {
            ...
            "trace_routes": r.trace_routes,
            "trace_gateways": r.trace_gateways,
