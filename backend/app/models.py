...
from sqlalchemy import Boolean, Date, DateTime, Float, ForeignKey, Integer, String, Table, Text, Column, UniqueConstraint, func
...
class ServiceRequest(Base):
    ...
    location: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ai_title_suggestion: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    planned_close_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
...
class WikiRagDocument(Base):
    ...
    chunks: Mapped[list["WikiRagChunk"]] = relationship(
        "WikiRagChunk",
...
class WikiRagChunk(Base):
    __tablename__ = "wiki_rag_chunks"
    __table_args__ = (UniqueConstraint("document_id", "chunk_index", name="uq_wiki_rag_chunks_doc_idx"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    document_id: Mapped[int] = mapped_column(
        ForeignKey("wiki_rag_documents.id", ondelete="CASCADE"), index=True
    )
    ...
    # Rich RAG metadata (CORAX rows / wiki sections).
    ...
    document: Mapped["WikiRagDocument"] = relationship("WikiRagDocument", back_populates="chunks")
...
class Note(Base):
    """Shared project notes / plans for panel users."""
...
class NoteShare(Base):
    ...
    __table_args__ = (UniqueConstraint("note_id", "user_id", name="uq_note_shares_note_user"),)
...
class TicketHandlerConfig(Base):
    """Singleton admin config for AI ticket handler pipeline."""
    ...
    llm_provider: Mapped[str] = mapped_column(String(32), default="ollama")  # ollama | lm_studio
    llm_base_url: Mapped[str] = mapped_column(String(512), default="http://127.0.0.1:11434/v1")
    llm_model: Mapped[str] = mapped_column(String(255), default="")
    include_corax_knowledge: Mapped[bool] = mapped_column(Boolean, default=True)
    include_wiki_docs: Mapped[bool] = mapped_column(Boolean, default=True)
    auto_create_ticket: Mapped[bool] = mapped_column(Boolean, default=True)
    default_priority: Mapped[str] = mapped_column(String(32), default="normal")
    default_category: Mapped[str] = mapped_column(String(255), default="")
...
class TicketHandlerRun(Base):
    """Journal of ticket-handler pipeline executions (for admin charts)."""

    __tablename__ = "ticket_handler_runs"
...
class RiskSnapshot(Base):
    """Periodic fleet-health snapshot for Risk Center history."""

    __tablename__ = "risk_snapshots"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)
    fleet_health_score: Mapped[int] = mapped_column(Integer)
    average_risk_score: Mapped[float] = mapped_column(Float, default=0)
    computers_total: Mapped[int] = mapped_column(Integer, default=0)
    computers_critical: Mapped[int] = mapped_column(Integer, default=0)
    computers_high: Mapped[int] = mapped_column(Integer, default=0)
    computers_medium: Mapped[int] = mapped_column(Integer, default=0)
    computers_healthy: Mapped[int] = mapped_column(Integer, default=0)
...
class RiskFindingAck(Base):
    ...
    __tablename__ = "risk_finding_acks"
