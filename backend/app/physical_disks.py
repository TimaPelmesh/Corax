...
_MEDIA_ONLY_LABELS = frozenset({"SSD", "HDD", "SCM", "Другой", "Том", "Неизвестно"})


def physical_disk_matches(raw_payload: str | None, name: str) -> bool:
    """Совпадение по типу носителя (SSD/HDD) или по точному варианту «SSD 240 ГБ»."""
    target = (name or "").strip()
    if not target:
        return False
    media_only = target in _MEDIA_ONLY_LABELS
    for row in disk_rows_from_raw_payload(raw_payload):
        ...
        if disk_variant_label(media, disk_size_label(size_val)) == target:
            return True
    return False


def aggregate_pc_disk_catalog(raw_payloads: list[str | None]) -> list[tuple[str, int]]:
    """Сводка по ПК: сначала типы носителя (SSD/HDD), затем варианты с объёмом."""
    from collections import Counter

    by_media_pcs: Counter[str] = Counter()
    by_variant_pcs: Counter[str] = Counter()
    for raw in raw_payloads:
        rows = disk_rows_from_raw_payload(raw)
        if not rows:
            continue
        medias: set[str] = set()
        ...
        for row in rows:
            ...
            size_val = float(size_gb) if isinstance(size_gb, (int, float)) else None
            medias.add(media)
            variants.add(disk_variant_label(media, disk_size_label(size_val)))
        for media in medias:
            by_media_pcs[media] += 1
        for variant in variants:
            # Не дублируем чистый SSD/HDD — он уже в блоке типов.
            if variant not in _MEDIA_ONLY_LABELS:
                by_variant_pcs[variant] += 1

    media_rows = sorted(by_media_pcs.items(), key=lambda item: (media_sort_key(item[0])[0], -item[1], item[0]))
    variant_rows = sorted(
        by_variant_pcs.items(),
        key=lambda item: (disk_variant_sort_key(item[0])[0], disk_variant_sort_key(item[0])[1], -item[1], item[0]),
    )
    return media_rows + variant_rows
