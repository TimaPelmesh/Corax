...
def _lldp_rem_index(key: str) -> str | None:
...
def _parse_lldp_man_ipv4(key: str, value: Any) -> str | None:
    """Extract IPv4 from lldpRemManAddr walk value or OID suffix."""
    ip = _bytes_or_str_ip(value)
    ...
    *,
    ...
    try:
        ...
        for key, val in lldp_man.items():
            ...
            ip = _parse_lldp_man_ipv4(key, val)
            if idx and ip:
                man_by_rem.setdefault(idx, ip)
