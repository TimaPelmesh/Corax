...
def _row_to_out(
    ...
    *,
    ...
    assignees: dict[int, list[tuple[int, str]]],
) -> ServiceRequestOut:
    created_by = users.get(row.created_by_id)
    computer = computers.get(row.computer_id) if row.computer_id else None
    assignee_rows = assignees.get(row.id, [])
    return ServiceRequestOut(
        ...
        ai_title_suggestion=getattr(row, "ai_title_suggestion", None),
        ai_enriched_at=getattr(row, "ai_enriched_at", None),
...
async def _to_out_many(db: AsyncSession, rows: list[ServiceRequest]) -> list[ServiceRequestOut]:
    if not rows:
        return []
    user_ids = {row.created_by_id for row in rows}
    computer_ids = {row.computer_id for row in rows if row.computer_id}
    ...
    computers: dict[int, Computer] = {}
    if computer_ids:
        cr = await db.execute(select(Computer).where(Computer.id.in_(computer_ids)))
        computers = {c.id: c for c in cr.scalars().all()}

    assignees: dict[int, list[tuple[int, str]]] = {rid: [] for rid in request_ids}
    ar = await db.execute(
        select(service_request_assignees.c.request_id, User.id, User.username)
        .join(User, User.id == service_request_assignees.c.user_id)
        .where(service_request_assignees.c.request_id.in_(request_ids))
        .order_by(User.username.asc())
    )
    for request_id, uid, username in ar.all():
        assignees.setdefault(int(request_id), []).append((int(uid), str(username)))

    return [
        _row_to_out(row, users=users, computers=computers, assignees=assignees)
        for row in rows
    ]


async def _to_out(db: AsyncSession, row: ServiceRequest) -> ServiceRequestOut:
    return (await _to_out_many(db, [row]))[0]
...
async def list_service_requests(
    ...
    out = await _to_out_many(db, rows)
...
async def delete_service_request(
    ...
    row = await db.get(ServiceRequest, request_id)
    if row:
        await delete_search_document(db, "service_request", row.id)
        await db.delete(row)
        await db.commit()
    return {"ok": True}
...
async def delete_all_service_requests(
    ...
    await db.execute(text("DELETE FROM search_documents WHERE entity_type = 'service_request'"))
...
async def import_status(_: User = Depends(get_current_user)):
    from app.service_request_import import service_request_import_runner

    return service_request_import_runner.snapshot()
...
async def _start_import_job(
    *,
    ...
    expected_suffix = ".csv" if kind == "glpi_csv" else ".json"
    if not filename.lower().endswith(expected_suffix):
        raise HTTPException(status_code=400, detail=f"Ожидается файл {expected_suffix}.")
    raw = await file.read()
    try:
        return await service_request_import_runner.start(
            kind=kind,
            raw=raw,
            filename=filename,
            created_by_id=current.id,
...
@router.post("/import-glpi-csv", status_code=202)
async def import_glpi_csv(
    file: UploadFile = File(...),
    current: User = Depends(get_current_editor_or_superuser),
):
    return await _start_import_job(file=file, kind="glpi_csv", current=current)


@router.post("/import-json", status_code=202)
async def import_json(
    file: UploadFile = File(...),
    current: User = Depends(get_current_editor_or_superuser),
):
    return await _start_import_job(file=file, kind="corax_json", current=current)
