...
def _toner_min_percent(supplies: list[PrinterSupplyOut]) -> int | None:
    """Минимум по основным расходникам (тонер / ribbon), без батареи и печей."""
    skip = re.compile(r"waste|battery|батаре|fuser|печь|drum|барабан|kit|maintenance", re.I)
    prefer = re.compile(r"toner|тонер|cartridge|картридж|ribbon|лента|ink|чернил", re.I)
    preferred: list[int] = []
    other: list[int] = []
    for s in supplies:
        if s.level_percent is None:
            continue
        name = s.name or ""
        if skip.search(name):
            continue
        if prefer.search(name):
            preferred.append(int(s.level_percent))
        else:
            other.append(int(s.level_percent))
...
def _printer_out(row: Printer, hostname: str | None = None) -> PrinterOut:
    ...
    return PrinterOut(
        ...
        printer_kind=getattr(row, "printer_kind", None),
        serial_number=getattr(row, "serial_number", None),
        snmp_sys_name=getattr(row, "snmp_sys_name", None),
...
async def delete_printer(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Принтер не найден")
    await delete_search_document(db, "printer", row.id)
    await db.delete(row)
    await db.commit()
