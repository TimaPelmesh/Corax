...
async def list_computers(
    ...
    if tag_ids:
        ...
        stmt = stmt.where(Computer.id.in_(tagged_computers))
...
_PC_CSV_ALIASES: dict[str, tuple[str, ...]] = {
    "name": ("Наименование", "Название", "Имя", "hostname", "host", "computer", "Компьютер"),
    "org": ("Организация", "Локация", "Расположение", "location", "organization"),
    "status": ("Статус", "status"),
    "manufacturer": ("Производитель", "manufacturer", "vendor"),
    "model": ("Модель", "model"),
    "os": (
    ...
    "cpu": ("Компоненты - Процессоры", "Процессор", "Процессоры", "cpu"),
    "notes": ("Примечание", "Заметки", "Комментарий", "notes", "comment"),
}
...
def _pc_csv_reader(text: str) -> tuple[csv.DictReader, dict[str, str | None]]:
    reader = csv.DictReader(io.StringIO(text), dialect=_detect_csv_dialect(text))
    ...
    for field, aliases in _PC_CSV_ALIASES.items():
        found: str | None = None
        ...
        if found is None:
            for norm_header, original in normalized.items():
                if any(norm_header.startswith(f"{alias} ") for alias in normalized_aliases):
                    found = original
                    break
        columns[field] = found

    if columns["name"] is None:
        shown = ", ".join(headers[:8]) or "—"
        raise ValueError(
            "Не найдена колонка с именем ПК. Поддерживаются CSV из GLPI и экспорт CORAX. "
            f"Колонки файла: {shown}"
        )
    return reader, columns


def _csv_cell(row: dict[str, str], column: str | None) -> str:
    if column is None:
        return ""
...
async def export_glpi_pcs_csv(_: User = Depends(get_current_user)):
    ...
    try:
        reader, columns = _pc_csv_reader(text)
    ...
    agg: dict[str, dict] = {}
    for row in reader:
        name = _csv_cell(row, columns["name"])
        if not name:
            continue
        key = name.strip()
        dt = _parse_glpi_pc_dt(_csv_cell(row, columns["last_change"]))
        cur = agg.get(key)
        if cur is None:
            cur = {
                "name": key,
        ...
        def pick(field_key: str, source_col: str | None):
            v = _csv_cell(row, source_col)
            if not v:
                return
        ...
        pick("org", columns["org"])
        pick("status", columns["status"])
        pick("manufacturer", columns["manufacturer"])
        pick("model", columns["model"])
        pick("os", columns["os"])
        pick("inv", columns["inv"])
        pick("type", columns["type"])
        pick("cpu", columns["cpu"])
...
async def update_computer(
    ...
    if not c:
        raise HTTPException(status_code=404, detail="ПК не найден")
    # exclude_unset: null в JSON = явное очищение; отсутствие поля = не трогать
    patch = body.model_dump(exclude_unset=True)
    if "notes" in patch:
        c.notes = (patch["notes"] or "").strip() or None
    if "location" in patch:
        c.location = (patch["location"] or "").strip() or None
...
async def import_glpi_pcs_csv(
    ...
    for hn, r in agg.items():
        ...
        if notes is None and notes_bits:
