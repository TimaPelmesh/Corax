...
def test_reject_windows_workstation():
    ...
    assert c.device_type in NETWORK_DEVICE_TYPES


def test_classify_eltex_switch():
    c = classify_device("Eltex MES2324B")
    assert c.device_type == "switch"
    assert c.is_network_gear


def test_classify_udm_router():
    c = classify_device("UniFi Dream Machine Pro", sys_name="udm-office-01")
    assert c.device_type == "router"
...
def test_classify_hostname_leaf_switch():
    c = classify_device("Linux", sys_name="leaf-01.lab")
    assert c.device_type == "switch"


def test_classify_wifi_controller():
    c = classify_device("Cisco Wireless Controller 5520 Software")
    assert c.device_type == "controller"


def test_classify_ats_voip():
    c = classify_device("Asterisk PBX 18.0", sys_name="ats-office-01")
