...
def test_parse_model_context_prefers_modelfile_num_ctx():
    payload = {
        "parameters": "num_ctx 16384\ntemperature 0.2",
        "model_info": {"qwen2.context_length": 32768},
    }
    assert _parse_model_context_length(payload) == 16384
...
def test_detect_llm_provider():
    assert detect_llm_provider("http://127.0.0.1:11434/v1") == "ollama"
    assert detect_llm_provider("http://192.168.1.10:11434") == "ollama"
    assert detect_llm_provider("http://ollama.lan:8080/v1") == "ollama"
    assert detect_llm_provider("http://127.0.0.1:1234/v1") == "lm_studio"
    assert detect_llm_provider("http://example.com:8080/v1") == "openai_compat"
    assert llm_provider_label("ollama") == "Ollama"
