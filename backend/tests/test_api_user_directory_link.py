...
def test_link_local_account_to_ad_directory(client: TestClient, auth_headers: dict[str, str]):
    """Seed AD person via TestClient portal (same event loop) — avoid AsyncSessionLocal + pytest-asyncio clash."""
    ad_username = unique_hostname("ad-link")
    local_username = unique_hostname("local-link")

    async def _seed_ad_directory_user() -> int:
    ...
    ad_id = client.portal.call(_seed_ad_directory_user)
...
def test_cannot_link_to_another_local_account(client: TestClient, auth_headers: dict[str, str]):
