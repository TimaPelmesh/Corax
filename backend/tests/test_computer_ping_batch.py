...
def test_resolve_ip_uses_column_only():
    from app.computer_ping import _resolve_ip

    c = MagicMock()
    c.ip_address = None
    c.raw_payload = '{"extended":{"network":{"adapters":[{"ipv4":["10.1.2.3"]}]}}}'
    c.hostname = "pc-hidden"
    c.mac_primary = "aa:bb:cc:dd:ee:ff"
    assert _resolve_ip(c) is None

    c.ip_address = "10.0.0.5"
    assert _resolve_ip(c) == "10.0.0.5"
