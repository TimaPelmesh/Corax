...
def test_risk_ai_insights_maps_connect_error_to_503(
    client: TestClient,
    auth_headers: dict[str, str],
    monkeypatch,
):
    ...
    monkeypatch.setattr("app.routers.risks.ensure_model_num_ctx", skip_ctx)
    monkeypatch.setattr("app.routers.risks.lm_studio_chat", boom)
    response = client.post(
        "/api/v1/risks/ai-insights",
        headers=auth_headers,
        json={"response_mode": "fast", "force": True, "base_url": "http://127.0.0.1:11434/v1"},
    )
