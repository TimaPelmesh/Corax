...
def test_agent_inventory_drops_placeholder_motherboard(
    client: TestClient, agent_headers: dict[str, str], auth_headers: dict[str, str]
):
    hn = unique_hostname("linux-mb")
    body = sample_inventory(hn)
    body["os_name"] = "Ubuntu 24.04 LTS"
    body["motherboard_manufacturer"] = "None"
    body["motherboard_product"] = "Default string"
    body["ram_gb"] = 16.0
    created = client.post("/api/v1/agent/inventory", json=body, headers=agent_headers)
    assert created.status_code == 200, created.text
    computer_id = created.json()["computer_id"]
    ...
    assert d["motherboard_manufacturer"] is None
    assert d["motherboard_product"] is None
    client.delete(f"/api/v1/computers/{computer_id}", headers=auth_headers)
